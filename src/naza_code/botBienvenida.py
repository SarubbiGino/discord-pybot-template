...
class Welcome_Embed():
    ...
    def enviar(self):
        self.embed = discord.Embed(
            title=f"Hola {self.member}. Para unirte al servidor debes aceptar las siguientes reglas...", colour=int("DC75FF", 16))
        self.embed.add_field(
            name="Reglas:", value="1. Ser respetuoso. / 2. No usar lenguaje inapropiado. / 3. No hacer spam. / 4. No server raiding. ", inline=False)
        self.embed.add_field(
            name="Escribe el siguiente comando si estas de acuerdo:", value="acepto", inline=False)
        return self.embed
...
def main():
    ...
    async def enviar_recordatorio():
        ...
        while not bot.is_closed():
            # Cambia esto para establecer la fecha límite deseada
            fecha_entrega = datetime.datetime.now() + datetime.timedelta(days=7)
            # Reemplaza "ID_DEL_CANAL_GENERAL" con el ID del canal general de tu servidor de Discord.
            canal_general = bot.get_channel(ID)
            ...
            # Establece la frecuencia deseada del recordatorio en segundos (en este caso, un día completo)
    ...
    async def comandos(ctx):
        ...
        embed.set_thumbnail(
            url="https://www.clipartmax.com/png/middle/72-720707_imagenes-sin-fondo-renders-png-renders-sin-fondo.png")
